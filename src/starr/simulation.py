...
class Simulation():
    ...
    def arange_regular_grid(self, grid_kws, group='all', checkerboard=False):
        """
        ...
        grid = Grid(origin, spacing, ranges, gradient=gradient, checkerboard=checkerboard)
        #grid = order_blockwise_radially(grid)
        grid_points = grid.lattice_sites
        total_particles = 0
        if group == 'all':
            total_groups = np.inf
            current_group = 0
        ...
        for row in range(grid_points.shape[0]):
            ...
            for ig, obj in enumerate(group.objects):
                ...
                obj.position = grid_points[total_particles, :]
                obj.update(0.0, self.world, self.canvas)
                total_area += obj.geometry.area()
                total_particles += 1
            n_groups_added += 1
            current_group += 1
            if n_groups_added == total_groups:
                break
